...
description: "Full configuration schema for litmdx.config.ts. All options with types, defaults, and examples: title, description, siteUrl, nav, head, openGraph, SEO, and more."
sidebar_position: 2
---
...
| `logo` | `string \| ThemeAsset` | `undefined` | Image URL/path for the header logo. LitMDX emits a `<link rel="preload" fetchpriority="high">` in `<head>` for the logo asset(s) to improve LCP. |
...
| `webmcp` | `boolean` | `false` | Register [WebMCP](/features/webmcp) tools on `navigator.modelContext` for AI agent integration |
...
Controls which heavy built-in components are included in the bundle. By default all are excluded to keep the initial bundle small.
...
| Field | Type | Default | Description |
|---|---|---|---|
| `mermaid` | `boolean` | `false` | Bundle the `<Mermaid>` component and consolidate its dependencies into a single chunk |

<Callout type="tip">
  Only enable components you actually use in your docs. Each opt-in component adds its dependency tree to the client bundle.
</Callout>
...
<Callout type="note">
`og:title`, `og:description`, `og:url`, and `<link rel="canonical">` are all updated per-route automatically by the `usePageMeta` hook on every client-side navigation. No extra configuration needed.
</Callout>
...
export default defineConfig({
  ...
  components: {
    mermaid: false, // set to true if your docs use <Mermaid> diagrams
  },
